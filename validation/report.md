...
**Retrieval on: 100.0%  retrieval off: 100.0%  delta: +0.0%**  (n=15 in-distribution incidents per condition)
...
|---|---|---|---|---|---|---|
| `rag_on` | **100.0%** (15/15) | 80.0% | 100.0% (n=15) | 0 | 0 | 15 |
| `rag_off` | **100.0%** (15/15) | 73.3% | 100.0% (n=15) | 0 | 0 | 15 |
...
Against the labelled incident->runbook mapping, `rag_on` only, n=15 in-distribution incidents.

- **Precision@5: 0.533** (ceiling 0.80)
- **MRR: 0.778**
...
|---|---|---|---|
| `rag_on` | **13.3%** (2/15) | 0.0% (0/15) | 9 / 4 |
| `rag_off` | **0.0%** (0/15) | 0.0% (0/15) | 11 / 4 |
...
|---|---|---|---|---|---|
| `rag_on` | 12.16s | 19.15s | 20.96s | 30 | within |
| `rag_off` | 8.46s | 15.33s | 15.84s | 30 | within |
...
|---|---|---|
| `rag_on` | 100.0% (28/28) | 28 |
| `rag_off` | 100.0% (30/30) | 30 |

## 6. Citation validity

- **Mean citation validity: 97.8%** (n=15 in-distribution `rag_on` responses that cited anything)
- Responses with at least one fabricated citation: 6.7%
...
Scatter data: `validation/results/relevance_vs_correctness.csv` (30 graded `rag_on` points).
...
- Only successes present among in-distribution graded rows (n=15), so no separation can be computed.
...
> n=15 in-distribution points. No correlation coefficient is quoted: at this n it would be dominated by individual observations.
...
- **Runs passed: 5/5** - the second diagnosis of an identical incident context cites the memorised chunk from the first.

- **Mean relevance increase: +0.280** (n=5 paired runs)
- Range: +0.203 to +0.374

> This is headline differentiator measured rather than asserted: the corpus learns from a resolved incident, and the next occurrence retrieves it. Paired design - each run compares the same context against itself before and after memorisation.

---

## 9. Grading provenance

- Rows graded: **60** from `grades.csv`
- Resolved mechanically by rubric: **57**
- Resolved by a human: **3**
- Still `NEEDS_HUMAN`: **0** (0%)

Manual grades and their stated reasons:

- `inc-cpu-throttle-004` / `rag_off` -> **PARTIALLY_CORRECT** - identifies CPU starvation and an actionable fix but never the bursty hourly batch spike that is the actual cause
- `inc-crashloop-003` / `rag_off` -> **PARTIALLY_CORRECT** - describes the startup symptom but does not commit to the command/entrypoint being wrong rather than another startup fault
- `inc-oom-005` / `rag_on` -> **CORRECT** - names the JVM heap setting as the cause and proposes -Xmx below the container limit; the memory limit is cited only as sizing context
